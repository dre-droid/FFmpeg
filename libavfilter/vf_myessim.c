...
static const AVOption ssim_options[] = {
    ...
    { "int_image", "Compute SSIM using integral images", OFFSET(int_image), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, FLAGS },
    { NULL }
};
...
// computes integral images i1, i2, s1, s2, i12, storing them in the context
static void compute_int_images(SSIMContext *s, uint8_t *master, uint8_t *ref, int main_linesize, int ref_linesize){
    uint64_t v = 0, a;

    for (int h = 0; h < s->planeheight[0]; h++)
        for (int w = 0; w < s->planewidth[0]; w++){
            s->i1[w + h * main_linesize] = v;
            v += master[w + h * main_linesize];
        }

    for (int h = 0; h < s->planeheight[0]; h++)
        for (int w = 0; w < s->planewidth[0]; w++){
            s->s1[w + h * main_linesize] = v;
            a = master[w + h * main_linesize];
            v += a * a;
        }

    for (int h = 0; h < s->planeheight[0]; h++)
        for (int w = 0; w < s->planewidth[0]; w++){
            s->i2[w + h * ref_linesize] = v;
            v += ref[w + h * ref_linesize];
        }

    for (int h = 0; h < s->planeheight[0]; h++)
        for (int w = 0; w < s->planewidth[0]; w++){
            s->s2[w + h * ref_linesize] = v;
            a = ref[w + h * ref_linesize];
            v += a * a;
        }

    for (int h = 0; h < s->planeheight[0]; h++)
        for (int w = 0; w < s->planewidth[0]; w++){
            s->i12[w + h * main_linesize] = v;
            v += master[w + h * main_linesize] * ref[w + h * ref_linesize];
        }
...
static int do_ssim(FFFrameSync *fs)
{
    ...
    if (s->int_image) {
        compute_int_images(s, master->data[0], ref->data[0], master->linesize[0], ref->linesize[0]);
    }
...
static av_cold int init(AVFilterContext *ctx)
{
    ...
    if (s->int_image){
        s->i1 = av_malloc_array(s->planewidth[0] * s->planeheight[0], sizeof(uint64_t));
        s->i2 = av_malloc_array(s->planewidth[0] * s->planeheight[0], sizeof(uint64_t));
        s->s1 = av_malloc_array(s->planewidth[0] * s->planeheight[0], sizeof(uint64_t));
        s->s2 = av_malloc_array(s->planewidth[0] * s->planeheight[0], sizeof(uint64_t));
        s->i12 = av_malloc_array(s->planeheight[0] * s->planeheight[0], sizeof(uint64_t));
    }
